...
class HarvestEngine:
    ...
    def harvest_plants(
        self, target_count: int = 5, existing_plants_list: List[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for name in plant_names:
            try:
                ...
                logger.info(
                    f"Successfully processed: {structured_data.get('common_name', name)}"
                )
    ...
    def start_background_harvesting(
        ...
        def worker():
            ...
            logger.info(
                f"Starting background harvesting for {days} days ({plants_per_hour} plants/hour). Ends at {end_time}"
            )
            ...
            while datetime.now() < end_time:
                try:
                    ...
                    harvested = self.harvest_plants(
                        target_count=plants_per_hour,
                        existing_plants_list=existing_plants,
                    )
    ...
    def _get_schema(self) -> str:
        return """
        {
            ...
            "plant_type": "annual | perennial | both",
            "description": "string",
    ...
    def _generate_plant_names(
        self, target_count: int, existing_names: List[str]
    ) -> List[str]:
        """
    ...
    def _fetch_wikipedia_image(self, plant_name: str) -> str:
        ...
        def fetch_by_title(title: str) -> str:
            try:
                ...
                req = urllib.request.Request(
                    url, headers={"User-Agent": "PlantHarvesterApp/1.0"}
                )
                ...
                logger.warning(
                    f"Error fetching Wikipedia image for title '{title}': {e}"
                )
            return ""
            ...
            try:
                ...
                with urllib.request.urlopen(req) as response:
                    ...
                    for _, page_info in pages.items():
                        ...
                        category_names = [
                            c.get("title", "").lower() for c in categories
                        ]
        ...
        for query in plant_queries:
            try:
                ...
                req = urllib.request.Request(
                    search_url, headers={"User-Agent": "PlantHarvesterApp/1.0"}
                )
                with urllib.request.urlopen(req) as response:
                    ...
                    if len(search_data) > 1 and len(search_data[1]) > 0:
                        for possible_title in search_data[1]:
                            if possible_title.strip() and is_plant_article(
                                possible_title
                            ):
                                fallback_image = fetch_by_title(possible_title)
                                if fallback_image:
                                    logger.info(
                                        f"Found plant image via search for '{plant_name}' using title '{possible_title}'"
                                    )
                                    return fallback_image
            except Exception as e:
                logger.warning(
                    f"Error during fallback search for '{plant_name}' with query '{query}': {e}"
                )
            ...
            with urllib.request.urlopen(req) as response:
                ...
                for result in search_results:
                    ...
                    img_req = urllib.request.Request(
                        img_url, headers={"User-Agent": "PlantHarvesterApp/1.0"}
                    )
                    with urllib.request.urlopen(img_req) as img_response:
                        ...
                        for _, page_info in pages.items():
                            if "thumbnail" in page_info:
                                ...
                                if not any(
                                    word in title_lower for word in exclude_words
                                ):
                                    logger.info(
                                        f"Found Wikimedia Commons image for '{plant_name}': {title}"
                                    )
                                    return page_info["thumbnail"]["source"]
        except Exception as e:
                    ...
                    if result.get("taxon"):
                        ...
                        if taxon.get("default_photo"):
                            ...
                            if photo.get("medium_url"):
                                logger.info(
                                    f"Found iNaturalist image for '{plant_name}'"
                                )
        ...
        try:
            ...
            with urllib.request.urlopen(req) as response:
                ...
                if data.get("usageKey"):
                    ...
                    media_url = (
                        f"https://api.gbif.org/v1/species/{data.get('usageKey')}/media"
                    )
                    media_req = urllib.request.Request(
                        media_url, headers={"User-Agent": "PlantHarvesterApp/1.0"}
                    )
                    with urllib.request.urlopen(media_req) as media_response:
                        ...
                        for media in media_results:
                            ...
                            if media.get("identifier"):
                                ...
                                if media.get("type") == "StillImage" or media.get(
                                    "format", ""
                                ).startswith("image/"):
...
if __name__ == "__main__":
    ...
    if not api_key:
        print(
            "ERROR: OPENAI_API_KEY environment variable not found. Please set it in your .env file."
        )
        exit(1)
    ...
    if len(sys.argv) > 1:
        ...
        harvested = engine.harvest_plants(
            target_count=2, existing_plants_list=mock_existing_db
        )
        if harvested:
