...
class PlantScheduler:
    ...
    def _build_prompt(self, garden_data: Dict[str, Any]) -> str:
        """
...
The garden is located in {garden_data.get('location', 'an unknown location')} and has {garden_data.get('sunlight', 'unknown')} sunlight.
...
"""
    ...
    def generate_schedule(self, garden_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Generates a task schedule based on the provided garden data.
        Returns a parsed JSON dictionary.
        """
        logger.info(f"Generating schedule for garden: {garden_data.get('name', 'Unknown')} using {self.model}")
        
        prompt = self._build_prompt(garden_data)
